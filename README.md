...
현재 구현 기준으로 보면 이 프로젝트는 Java Core Global Percentile 기준 **Top 59-40 구간에 진입한 학습 프로젝트**에 가깝습니다.

구체적으로는 **Top 55-45% 전후**로 볼 수 있습니다.

이유는 다음과 같습니다.

- 단순 문법 사용을 넘어 클래스를 역할별로 분리했습니다.
- `Product`, `Cart`, `Customer`, `Grade`처럼 도메인 객체를 만들었습니다.
- `IProducts` 인터페이스로 카테고리 상품 로딩 규약을 분리했습니다.
- `Grade` enum으로 회원 등급과 할인율을 관리했습니다.
- `ListManager`, `DialogManager`를 통해 콘솔 출력과 데이터 흐름을 일부 분리했습니다.
- 여러 커스텀 예외를 만들어 상황별 오류 처리를 시도했습니다.
- 장바구니, 주문, 잔액, 재고 차감 흐름을 하나의 서비스 흐름으로 연결했습니다.

다만 아직 Top 39-20 구간으로 보기에는 다음 부분이 더 필요합니다.
